...
public class typeof_ColorTexture_InferenceRule extends AbstractInferenceRule_Runtime implements InferenceRule_Runtime {
  ...
  public void applyRule(final SNode colorTexture, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    {
      final SNode strokeType = typeCheckingContext.typeOf(SLinkOperations.getTarget(colorTexture, LINKS.stroke$FXvv), "r:34dc5c2b-d71f-4a9a-9011-74cd28ad1a10(jetbrains.mps.samples.Physics.typesystem)", "7101899341841108510", true);
      typeCheckingContext.whenConcrete(strokeType, new Runnable() {
        public void run() {
          if (!(typeCheckingContext.isSingleTypeComputation())) {
            {
              SNode _nodeToCheck_1029348928467 = SLinkOperations.getTarget(colorTexture, LINKS.stroke$FXvv);
    ...
    {
      final SNode colorType = typeCheckingContext.typeOf(SLinkOperations.getTarget(colorTexture, LINKS.color$CDRp), "r:34dc5c2b-d71f-4a9a-9011-74cd28ad1a10(jetbrains.mps.samples.Physics.typesystem)", "7101899341841111107", true);
      typeCheckingContext.whenConcrete(colorType, new Runnable() {
        public void run() {
          if (!(typeCheckingContext.isSingleTypeComputation())) {
            {
              SNode _nodeToCheck_1029348928467 = SLinkOperations.getTarget(colorTexture, LINKS.color$CDRp);
