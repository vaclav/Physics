      ...
      public DVector3C linearForce(World world, SystemScope scope, PhysicalEntity currentEntity, double time) {
        cached = InteractionForceForce.get(world, scope, currentEntity, time, new Function<ParameterSetWrapper, VectorLike>() {
          public VectorLike apply(ParameterSetWrapper param) {
            ...
            return otherObject.minus(currentEntity).resize(AH.div(AH.mul(AH.mul(AH.mul(((Number) new BigInteger("9")), ((Number) new BigInteger("1"))), currentEntity.getMass()), otherObject.getMass()), BigDecimal.valueOf(Math.pow(currentEntity.minus(otherObject).length().doubleValue(), ((Number) new BigInteger("2")).doubleValue()))));
          }
        });
