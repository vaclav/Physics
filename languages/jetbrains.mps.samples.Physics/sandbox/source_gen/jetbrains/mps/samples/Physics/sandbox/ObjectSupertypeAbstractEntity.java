...
public abstract class ObjectSupertypeAbstractEntity<T extends SystemScope> extends BaseObjectAbstractEntity<T> {
  ...
  public void init(T scope, World world) {
    ...
    this.getForces().addAll(Arrays.asList(new Force<SystemScope>() {
      ...
      public DVector3C linearForce(World world, SystemScope scope, PhysicalEntity currentEntity, double time) {
        if (cached == null) {
          cached = InternalVector.fromSpherical(AH.mul(((Number) new BigDecimal("0.47114206162369554").setScale(17, RoundingMode.DOWN)), ((Number) (AH.mul(BigDecimalMath.pi(MathContext.DECIMAL32), ((Number) new BigInteger("1")))))), AH.mul(((Number) new BigDecimal("0.5").setScale(1, RoundingMode.DOWN)), ((Number) (AH.mul(BigDecimalMath.pi(MathContext.DECIMAL32), ((Number) new BigInteger("1")))))), ((Number) (AH.mul(((Number) new BigDecimal("44.1814440687490434227723411264442").setScale(31, RoundingMode.DOWN)), ((Number) new BigInteger("1"))))));
        }
