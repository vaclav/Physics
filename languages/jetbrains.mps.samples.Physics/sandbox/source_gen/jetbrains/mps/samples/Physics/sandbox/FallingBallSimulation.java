...
public class FallingBallSimulation extends Simulation {
  ...
  protected void renderMetrics(PGraphics ctx) {
    ...
    this.metricsRenderer.renderMetric(ctx, "Potention Energy", MetricsRenderer.anyToString(AH.mul(AH.mul(AH.mul(((Number) new BigDecimal("9.81").setScale(2, RoundingMode.DOWN)), new BigDecimal("1")), BigDecimalHelper.of(AH.sub(scope.Ball.getPosition().getY(), scope.Ground.getPosition().getY())).abs()), scope.Ball.getMass())) + " s^-2 * kg^1 * m^2");
