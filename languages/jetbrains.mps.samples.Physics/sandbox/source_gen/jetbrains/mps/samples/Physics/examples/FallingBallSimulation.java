...
public class FallingBallSimulation extends Simulation {
  ...
  protected void renderMetrics(PGraphics ctx) {
    ...
    this.metricsRenderer.renderMetric(ctx, "Kinetic Energy", MetricsRenderer.anyToString(AH.mul(AH.mul(AH.div(((Number) new BigDecimal("1.0").setScale(1, RoundingMode.DOWN)), ((Number) new BigInteger("2"))), BigDecimal.valueOf(Math.pow(scope.Ball.getVelocity().length().doubleValue(), ((Number) new BigInteger("2")).doubleValue()))), scope.Ball.getMass())) + " m^2 * s^-2 * kg^1");
    this.metricsRenderer.renderMetric(ctx, "Potention Energy", MetricsRenderer.anyToString(AH.mul(AH.mul(AH.mul(((Number) new BigDecimal("9.81").setScale(2, RoundingMode.DOWN)), ((Number) new BigInteger("1"))), BigDecimalHelper.of(AH.sub(scope.Ball.getPosition().getY(), scope.Ground.getPosition().getY())).abs()), scope.Ball.getMass())) + " m^2 * s^-2 * kg^1");
    this.metricsRenderer.renderMetric(ctx, "Total energy", MetricsRenderer.anyToString(AH.add(AH.mul(AH.mul(AH.div(((Number) new BigDecimal("1.0").setScale(1, RoundingMode.DOWN)), ((Number) new BigInteger("2"))), BigDecimal.valueOf(Math.pow(scope.Ball.getVelocity().length().doubleValue(), ((Number) new BigInteger("2")).doubleValue()))), scope.Ball.getMass()), AH.mul(AH.mul(AH.mul(((Number) new BigDecimal("9.81").setScale(2, RoundingMode.DOWN)), ((Number) new BigInteger("1"))), BigDecimalHelper.of(AH.sub(scope.Ball.getPosition().getY(), scope.Ground.getPosition().getY())).abs()), scope.Ball.getMass()))) + " m^2 * s^-2 * kg^1");
  }
