...
import MetricsRenderer from "./objects/rendering/MetricsRenderer";
...
export abstract class Simulation implements RendererCallback {
  ...
  public setup(renderer: p5): void {
    ...
    this.metricsRenderer = new MetricsRenderer(renderer);
  ...
  public render(applet: p5, ctx: p5.Graphics, glContext: WebGLRenderingContextBase): void {
    ctx.background(0);
    ...
    // ctx.textMode(p5.MODEL);

    this.metricsRenderer!.renderDefault(this.world, ctx);
  ...
  protected abstract renderMetrics(ctx: p5.Graphics): void;
}
