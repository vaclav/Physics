#!/bin/sh
# ---------------------------------------------------------------------
# Physics startup script.
# ---------------------------------------------------------------------
# Generated by MPS

message()
{
  TITLE="Cannot start Physics"
...
# Try (in order): $Physics_JDK, .../physics.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$Physics_JDK" ] && [ -x "$Physics_JDK/bin/java" ]; then
  JDK="$Physics_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Physics1.0/physics.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Physics1.0/physics.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either Physics_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$Physics_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$Physics_PROPERTIES"
fi
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Physics1.0/physics$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/Physics1.0/physics$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/physics$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/physics$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/physics$BITS.vmoptions"
fi
...
# shellcheck disable=SC2154
if [ -n "$Physics_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$Physics_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=Physics1.0
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_Physics_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_Physics.hprof" \
